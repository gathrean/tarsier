...
> **Target:** iOS 17+, iPhone only (no iPad layout for v1.0)

> **Reference docs (read these when you need context):**  
> - `docs/Tarsier_Roadmap.md` — 32-lesson curriculum (10 chapters), row layouts, affix progression, onboarding flow, marketing timeline, chapters.json  
> - `docs/Tarsier_Research.md` — market research, competitor analysis, design decisions, audience insights  
> - `docs/Lessons/lesson_001_po.json` — Lesson 1 JSON schema v0.6 (Po & Opo, Chapter 1)  
...
Tarsier is a gamified Tagalog language learning app, "Duolingo for Tagalog." The core differentiator is teaching Tagalog's affix/etymology system (how prefixes and suffixes transform root words) rather than isolated flashcards. Lessons use a session-based flow with interleaved teach-then-quiz cards, inspired by AirLearn and HelloChinese.
...
### App Structure (Two-Level Hierarchy)
- **Chapter** = visual grouping label on roadmap (e.g., "Magalang, Respect & Greetings")
- **Lesson** = circle on roadmap with progress ring (5 sessions inside)
- Each lesson has its own name ("Po & Opo," "Kumusta"), not chapter numbering
- Roadmap rows: 1-2 lessons per row, creating visual rhythm
- Practice node at end of each chapter (AI practice)
- chapters.json defines row layout per chapter. See `docs/Tarsier_Roadmap.md`.

### Lesson JSON Schema (v0.6)
- Top level: lesson metadata, sessions array, vocabulary array, completion_reward, ai_practice, gamification
- Each session: cards array with interleaved teach and quiz cards
- Quiz types: multiple_choice, fill_in_blank, translate, root_pattern, word_order
- Optional fields on any card: `alam_mo_ba_inline` (contextual tooltip), `image` (teach cards only)
- `image` renders ONLY on teach cards. If file not in bundle, render nothing (no placeholder).
...
5. Session 1 complete: ring animates 1/5, Alam Mo Ba, +15 XP
6. NOW show home screen with roadmap
7. Paywall triggers on hearts empty or AI practice node
...
- This selection is stored in SwiftData but doesn't change lesson content in v1.0
...
- Layout:
  ...
  - Speaker icon (for future TTS — show but disable in v1.0 with "Coming soon" tooltip)
